...
def login(request, next_page=None, required=False):
    ...
    if not next_page:
	#print cas_user_authenticated('user') + "first"
    ...
    if request.user.is_authenticated():
        ...
        #print cas_user_authenticated(user) + "second"
        print "HERE"
        return HttpResponseRedirect(next_page)
    ...
    if ticket:
        ...
        if user is not None:
            ...
            if settings.CAS_LOGIN_MSG is not None:
                ...
                messages.success(request, message)
            try:
                listusers = {"staff":"academic/staff/","hiring_committee":"academic/hiring_committee/","faculty":"academic/faculty"} #definig redirect URLS
                u=User.objects.get(username=name)
                print next_page
                check = True
                for i in listusers:
                    if listusers[i] in next_page:
                        check = False
                if u.role.is_hiring_committee and check is True:
                    if "hiring_committee" not in next_page:
                        next_page+=listusers['hiring_committee']
                elif u.role.is_staff and check is True:
                    next_page+=listusers['staff']
                elif u.role.is_faculty and check is True:
                    next_page+=listusers['faculty']
                else:
                    next_page=next_page
            except:
                print "You are not assigned any role yet."
            return HttpResponseRedirect(next_page)
